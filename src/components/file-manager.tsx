...
import type { SortField, SortDirection, FileItem, FolderItem } from "@/types/file";
...
export default function FileManager() {
  ...
  const { currentPath, history, historyIndex, navigateToFolder, goBack, goForward } = useNavigation();
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="border-b">
        <div className="flex items-center p-2 gap-2">
          <NavigationControls canGoBack={historyIndex > 0} canGoForward={historyIndex < history.length - 1} onBack={goBack} onForward={goForward} />
          <FileBreadcrumb path={currentPath} onNavigate={handleBreadcrumbNavigate} />
          <div className="ml-auto">
            <Toolbar viewMode={viewMode} onViewModeChange={setViewMode} searchQuery={searchQuery} onSearchChange={setSearchQuery} />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        {isLoading ? (
          ...
          <GridView root={root} path={currentPath} searchQuery={searchQuery} onNavigate={handleNavigateToFolder} />
        )}
      </div>
    </div>
  );
